...
loadJSON fileName = do
    jsonString <- Data.ByteString.Lazy.readFile fileName
    return (decode jsonString)

validate :: String -> TuringMachine -> Maybe String
validate input tm = do
    ...
    else if (any (\l -> (JSONparsing.read l) `notElem` (alphabet tm)) (concat (elems (transitions tm)))) then
    ...
    else if (any (\l -> (JSONparsing.write l) `notElem` (alphabet tm)) (concat (elems (transitions tm)))) then
    ...
    else if (head (blank tm)) `elem` input then
...
parseTuring input fileName = do
    ...
    case turingMachine of
        Nothing -> do
            putStrLn "Error parsing JSON file"
            return Nothing
        Just tm -> do
